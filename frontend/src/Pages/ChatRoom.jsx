  ...
  return (
    ...
    <div className="flex  h-screen">
      ...
      <div className="flex flex-col w-1/4 border-r-2">
       ...
       {/* List of participants */}
        ...
        />
          ...
          <div className="w-full">
            <div className="text-lg font-semibold">{item?.username}<p className='text-base text-primary inline-flex mx-1'>{item?.pk===pk?' (you)':''}</p></div>
          </div>
        </div>
     ...
     {/* messages listing part */}
      <div className="px-5 w-3/4 border relative h-fit  min-h-screen">
        <div className=" mt-5  relative">
          {messageList?.map((item,indx)=>{
            ...
            return item?.sender_details?.pk===pk?(
              ...
              {item?.photo && (<img className='h-[300px] w-[300px]'  src={item?.photo}/>)}
              {item?.video && (<video width="320" height="240" controls>
                    <source src={item.video} type="video/mp4" />
                    Your browser does not support the video tag.
              </video>)}
              {item?.document && (      <a href={item?.document} target="_blank" rel="noreferrer" className='text-primary underline'>{item?.document.split('/').pop()}</a>)}
               {item?.content}
              </div>    
            </div>
        ...
        {/* message input */}
        <div className="py-2 border-t-2 border-b-2 border-primary z-50 bg-white rounded-xl w-[95%]">
      ...
      <p className='ml-1 mt-2 text-gray-700 cursor-pointer hover:bg-slate-300 p-1 rounded-md' onClick={()=>setShowPicker(!showPicker)}>
        ...
        <p className={`font-semibold mr-3 py-3${msgLoading ? 'cursor-pointer' :'cursor-wait'}`} onClick={sendMessage}>send</p>
        }
      </div>
